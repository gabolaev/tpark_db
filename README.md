...
Контейнер будет собираться из запускаться командами:
```
docker build -t g.gabolaev https://github.com/gabolaev/tpark_db.git
docker run -p 5000:5000 --name g.gabolaev -t g.gabolaev
```
